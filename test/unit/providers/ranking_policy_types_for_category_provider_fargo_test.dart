...
void main() {
  ...
  group(
      'rankingPolicyTypesForCategoryProvider – Fargo whitelist (sports/pubgames)',
      () {
    for (final catId in [kSportsCategoryId, kPubGamesCategoryId]) {
      test(
          '[$catId] try getByCategory -> mapped [fargoRate] when repo returns Fargo policies',
          () async {
        // Elo whitelist is unconditional – provider returns [elo] without repo call.
      ...
      test(
          '[$catId] empty getByCategory + getAll empty -> bootstrap [fargoRate]',
          () async {
      ...
      test(
          '[$catId] empty getByCategory + getAll non-empty -> [fargoRate] (Fargo whitelist unconditional)',
          () async {
        // Even when repo has data, Fargo still returns [elo] without querying repo.
        final result = await container
            .read(rankingPolicyTypesForCategoryProvider(catId).future);
      ...
      test(
          '[$catId] getByCategory throws -> bootstrap fallback when getAll empty',
          () async {
        // Unconditional – even throw case returns [elo] without repo call.
        final result = await container
            .read(rankingPolicyTypesForCategoryProvider(catId).future);
      ...
      test(
          '[$catId] getByCategory throws -> [fargoRate] when getAll non-empty (Fargo whitelist unconditional)',
          () async {
        final result = await container
            .read(rankingPolicyTypesForCategoryProvider(catId).future);
    ...
    test('non-Fargo non-custom still returns [] without repo call (boardgames)',
        () async {
