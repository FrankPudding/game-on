...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockRepo.get()).thenAnswer((_) async => LeagueSortPreference.defaultPreference);
    when(() => mockRepo.getPreference()).thenAnswer((_) async => LeagueSortPreference.defaultPreference);
  ...
  group('sortPreferenceProvider (R4 persisted preference, no flicker, R3 toggle)', () {
    test('default is lastPlayed descending (R1+R4 no-flicker)', () {
      ...
      expect(LeagueSortPreference.defaultPreference.mode, LeagueSortMode.lastPlayed);
      expect(LeagueSortPreference.defaultPreference.descending, true);
    });

    test('initial load uses repository get (or default) - no flicker synchronous fallback', () async {
      ...
      when(() => mockRepo.get()).thenAnswer((_) async => const LeagueSortPreference(mode: LeagueSortMode.alphabetical, descending: false));
      when(() => mockRepo.getPreference()).thenAnswer((_) async => const LeagueSortPreference(mode: LeagueSortMode.alphabetical, descending: false));
      final c2 = ProviderContainer(overrides: [leagueSortPreferenceRepositoryProvider.overrideWithValue(mockRepo)]);
    ...
    test('setPreference persists via repository and updates state (write-then-invalidate R7)', () async {
      final notifier = container.read(sortPreferenceProvider.notifier);
      const newPref = LeagueSortPreference(mode: LeagueSortMode.alphabetical, descending: false);
    ...
    test('setMode preserves descending and persists', () async {
      ...
      verify(() => mockRepo.set(any(that: isA<LeagueSortPreference>()))).called(greaterThanOrEqualTo(1));
    });
    ...
    test('toggleDirection flips descending and persists (R3)', () async {
      ...
      await notifier.toggleDirection();
      expect(container.read(sortPreferenceProvider).descending, true);
      verify(() => mockRepo.set(any(that: isA<LeagueSortPreference>()))).called(greaterThanOrEqualTo(2));
    });

    test('LeagueSortPreference copyWith preserves equality', () {
      const base = LeagueSortPreference(mode: LeagueSortMode.lastPlayed, descending: true);
      final copy = base.copyWith(descending: false);
      expect(copy.mode, LeagueSortMode.lastPlayed);
      expect(copy.descending, false);
      expect(copy == const LeagueSortPreference(mode: LeagueSortMode.lastPlayed, descending: false), isTrue);
      expect(base.copyWith().mode, base.mode);
    });

    test('Provider is not autoDispose? keepAlive implied - not disposed on unwatch', () {
      ...
      expect(sortPreferenceProvider is NotifierProvider<LeagueSortPreferenceNotifier, LeagueSortPreference>, isTrue);
    });

    test('Persistence round-trip: set then get returns same (via repo)', () async {
