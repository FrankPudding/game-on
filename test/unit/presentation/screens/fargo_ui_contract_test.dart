...
void main() {
  group('Fargo UI – CreateFargoRateLeagueScreen', () {
    testWidgets('CreateFargoRateLeagueScreen builds without throwing (skeleton throws UnimplementedError)', (tester) async {
  ...
  group('LeagueDetail standings – isFargo columns', () {
    ...
    testWidgets('StandingsTab with isFargo shows Fargo columns (P,W,L,Win%,Fargo) not Pts', (tester) async {
      final fargoStats = {
        'p1': const FargoPlayerStats(matchesPlayed: 1, wins: 1, losses: 0, rating: 510),
      };
      ...
      expect(fargoStats['p1']!.winRate, 1.0); // will fail with UnimplementedError against skeleton – intentional
  ...
  group('LogMatch – Fargo hides draw', () {
    test('winRate hides draw calculation – Fargo should not allow isDraw true', () {
      ...
      const stats = FargoPlayerStats(matchesPlayed: 2, wins: 1, losses: 1, rating: 500);
