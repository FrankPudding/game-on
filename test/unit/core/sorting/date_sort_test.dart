...
void main() {
  group('compareNullableDateNullLast (R1 null-last both directions)', () {
    ...
    test('null always last regardless of descending', () {
      ...
      expect(compareNullableDateNullLast(null, dNew, descending: true),
          greaterThan(0));
      expect(compareNullableDateNullLast(null, dNew, descending: false),
          greaterThan(0));
      // a non-null, b null => a before b => negative
      expect(compareNullableDateNullLast(dNew, null, descending: true),
          lessThan(0));
      expect(compareNullableDateNullLast(dNew, null, descending: false),
          lessThan(0));
    });

    test('descending true -> most recent first', () {
      // dNew vs dOld with descending true: dNew should sort before dOld => negative
      expect(compareNullableDateNullLast(dNew, dOld, descending: true),
          lessThan(0));
      expect(compareNullableDateNullLast(dOld, dNew, descending: true),
          greaterThan(0));
      expect(compareNullableDateNullLast(dMid, dMid, descending: true), 0);
    });

    test('descending false -> oldest first', () {
      expect(compareNullableDateNullLast(dOld, dNew, descending: false),
          lessThan(0));
      expect(compareNullableDateNullLast(dNew, dOld, descending: false),
          greaterThan(0));
    ...
    test('nulls last in list sort both directions', () {
      ...
      final desc = List<DateTime?>.from(dates)
        ..sort((a, b) => compareNullableDateNullLast(a, b, descending: true));
      expect(desc, [dNew, dMid, dOld, null, null]);

      final asc = List<DateTime?>.from(dates)
        ..sort((a, b) => compareNullableDateNullLast(a, b, descending: false));
