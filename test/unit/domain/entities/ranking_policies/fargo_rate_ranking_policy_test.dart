...
void main() {
  group('FargoRateRankingPolicy – categoryIds invariant (set-equality)', () {
    ...
    test('accepts exact set reversed order [pubgames, sports] (order-insensitive)', () {
      final p = FargoRateRankingPolicy(
        id: 'rp1',
        name: 'Fargo',
        leagueId: 'l1',
    ...
    test('validateCategoryIds accepts both orders', () {
      expect(() => FargoRateRankingPolicy.validateCategoryIds(const [kSportsCategoryId, kPubGamesCategoryId]), returnsNormally);
      expect(() => FargoRateRankingPolicy.validateCategoryIds(const [kPubGamesCategoryId, kSportsCategoryId]), returnsNormally);
    });
    ...
    test('rejects duplicate ids (e.g. [sports, sports])', () {
      ...
      expect(() => FargoRateRankingPolicy.validateCategoryIds(const [kSportsCategoryId, kSportsCategoryId]), throwsArgumentError);
    });
    ...
    test('rejects extra ids (3 elements)', () {
      expect(
        () => FargoRateRankingPolicy(
          ...
          name: 'Fargo',
          leagueId: 'l1',
          categoryIds: const [kSportsCategoryId, kPubGamesCategoryId, 'cat_extra'],
        ),
        throwsArgumentError,
      );
      expect(() => FargoRateRankingPolicy.validateCategoryIds(const [kSportsCategoryId, kPubGamesCategoryId, kFallbackCategoryId]), throwsArgumentError);
    });
    ...
    test('rejects empty', () {
      ...
      expect(() => FargoRateRankingPolicy.validateCategoryIds(const []), throwsArgumentError);
    });
