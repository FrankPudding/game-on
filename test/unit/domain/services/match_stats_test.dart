...
void main() {
  group('maxCompletePlayedAt (R1,R5,R6 domain purity)', () {
    ...
    test('ignores incomplete even when its playedAt is newest', () {
      final incompleteNewest = _make(
          id: 'm1',
          leagueId: 'l1',
          playedAt: DateTime(2023, 8, 1),
          isComplete: false);
      final completeOld = _make(
          id: 'm2',
          leagueId: 'l1',
          playedAt: DateTime(2023, 1, 10),
          isComplete: true);
      final completeMid = _make(
          id: 'm3',
          leagueId: 'l1',
          playedAt: DateTime(2023, 6, 15),
          isComplete: true);
      final result =
          maxCompletePlayedAt([incompleteNewest, completeOld, completeMid]);
    ...
    test('single completed returns its date', () {
      ...
      expect(
          maxCompletePlayedAt(
              [_make(id: 'm1', leagueId: 'l1', playedAt: d, isComplete: true)]),
          d);
    });

    test(
        'multiple leagues grouping scenario - max per league independent (uses helper per group)',
        () {
    ...
    test('R6 domain purity - no Hive/Flutter imports in implementation file',
        () async {
