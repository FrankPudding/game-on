...
void main() {
  ...
  group('FargoRateCalculator.calculate', () {
    test('returns map for all players even with no matches (rating 500)', () {
      final result = calculator.calculate(matches: [], players: [p1, p2]);
    ...
    test('filters where(isComplete) before sort – incomplete ignored not counted not throwing', () {
      ...
      final result = calculator.calculate(matches: [incomplete], players: [p1, p2]);
      expect(result['p1']!.matchesPlayed, 0);
      expect(result['p2']!.matchesPlayed, 0);
      expect(result['p1']!.rating, 500);
    });

    test('incomplete with invalid sides (draw, 1 side) does NOT throw because filtered', () {
      final badIncomplete = SimpleMatch(
        ...
        isComplete: false,
        isDraw: true,
        sides: [Side(id: 's1', playerIds: ['p1', 'p2'])],
      );
      expect(() => calculator.calculate(matches: [badIncomplete], players: [p1, p2]), returnsNormally);
    });
    ...
    test('throws ArgumentError on isDraw==true (complete)', () {
      ...
      expect(() => calculator.calculate(matches: [m], players: [p1, p2]), throwsArgumentError);
    });
    ...
    test('throws ArgumentError when sides.length !=2', () {
      final oneSide = SimpleMatch(
        ...
        playedAt: DateTime(2024, 1, 1),
        isComplete: true,
        sides: [Side(id: 's1', playerIds: ['p1'])],
      );
      expect(() => calculator.calculate(matches: [oneSide], players: [p1, p2]), throwsArgumentError);
      ...
      expect(() => calculator.calculate(matches: [threeSides], players: [p1, p2, p3]), throwsArgumentError);
    });
    ...
    test('throws ArgumentError when any playerIds.length !=1', () {
      ...
      expect(() => calculator.calculate(matches: [teamSide], players: [p1, p2, p3]), throwsArgumentError);
      ...
      expect(() => calculator.calculate(matches: [emptySide], players: [p1, p2]), throwsArgumentError);
    });

    test('sorts playedAt ASC + id ASC deterministically (winner rating affected by order)', () {
      ...
      final mA = SimpleMatch(
        ...
        playedAt: sameDate,
        isComplete: true,
        sides: [Side(id: 's_a1', playerIds: ['p1']), Side(id: 's_a2', playerIds: ['p2'])],
        winnerSideId: 's_a1',
      );
      final mB = SimpleMatch(
        ...
        playedAt: sameDate,
        isComplete: true,
        sides: [Side(id: 's_b1', playerIds: ['p1']), Side(id: 's_b2', playerIds: ['p2'])],
        winnerSideId: 's_b2',
      );

      final resultAB = calculator.calculate(matches: [mA, mB], players: [p1, p2]);
      final resultBA = calculator.calculate(matches: [mB, mA], players: [p1, p2]);
    ...
    test('playedAt ASC ordering before id', () {
      final early = _match(id: 'm1', playedAt: DateTime(2024, 1, 1), winnerSideId: 's_m1_1', winnerPlayerId: 'p1', loserPlayerId: 'p2');
      final late = _match(id: 'm2', playedAt: DateTime(2024, 1, 2), winnerSideId: 's_m2_1', winnerPlayerId: 'p2', loserPlayerId: 'p1');
      final result = calculator.calculate(matches: [late, early], players: [p1, p2]);
    ...
    test('updates 500 initial rating ELO-like: winner increases, loser decreases', () {
      final m = _match(id: 'm1', playedAt: DateTime(2024, 1, 1), winnerSideId: 's_m1_1', winnerPlayerId: 'p1', loserPlayerId: 'p2');
      final result = calculator.calculate(matches: [m], players: [p1, p2]);
    ...
    test('multiple matches: stats accumulate correctly', () {
      final m1 = _match(id: 'm1', playedAt: DateTime(2024, 1, 1), winnerSideId: 's_m1_1', winnerPlayerId: 'p1', loserPlayerId: 'p2');
      final m2 = _match(id: 'm2', playedAt: DateTime(2024, 1, 2), winnerSideId: 's_m2_1', winnerPlayerId: 'p1', loserPlayerId: 'p2');
      final result = calculator.calculate(matches: [m1, m2], players: [p1, p2]);
      expect(result['p1']!.matchesPlayed, 2);
    ...
    test('returns map for all players even with 3 players but only 2 involved', () {
      final m = _match(id: 'm1', playedAt: DateTime(2024, 1, 1), winnerSideId: 's_m1_1', winnerPlayerId: 'p1', loserPlayerId: 'p2');
      final result = calculator.calculate(matches: [m], players: [p1, p2, p3]);
    ...
    test('rating stays integer and not NaN after many matches', () {
      final matches = List.generate(5, (i) => _match(id: 'm$i', playedAt: DateTime(2024, 1, i + 1), winnerSideId: 's_m${i}_1', winnerPlayerId: 'p1', loserPlayerId: 'p2'));
      final result = calculator.calculate(matches: matches, players: [p1, p2]);
