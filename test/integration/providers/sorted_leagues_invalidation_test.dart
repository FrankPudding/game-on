...
void main() {
  ...
  final tPolicy1 = SimpleRankingPolicy(id: 'rp1', name: 'Standard', leagueId: 'l1');
  final tPolicy2 = SimpleRankingPolicy(id: 'rp2', name: 'Standard', leagueId: 'l2');
  ...
  setUp(() {
    ...
    when(() => mockPolicyRepo.getByLeagueId(any())).thenAnswer((_) async => tPolicy1);
    when(() => mockUserRepo.getAll()).thenAnswer((_) async => []);
    when(() => mockUserRepo.get(any())).thenAnswer((_) async => null);
    when(() => mockCreateService.execute(id: any(named: 'id'), name: any(named: 'name'), rankingPolicy: any(named: 'rankingPolicy'))).thenAnswer((_) async => {});
  ...
  group('SortedLeaguesProvider explicit invalidation graph (R7)', () {
    test('leagues addLeague invalidates sortedLeaguesProvider', () async {
      ...
      final sub = container.listen(sortedLeaguesProvider, (_, __) { invalidateCount++; });
      // Simulate addLeague which should invalidate sortedLeaguesProvider
      final newLeague = League(id: 'l3', name: 'Gamma', createdAt: DateTime.now());
      when(() => mockLeagueRepo.getAll()).thenAnswer((_) async => [tLeague1, tLeague2, newLeague]);
      await container.read(leaguesProvider.notifier).addLeague(id: 'l3', name: 'Gamma', rankingPolicy: SimpleRankingPolicy(id: 'rp3', name: 'S', leagueId: 'l3'));
    ...
    test('logSimpleMatch invalidates sortedLeaguesProvider and recomputes lastPlayed', () async {
      // Initial: no matches -> both Never
      await container.read(sortedLeaguesProvider.future);
      var count = 0;
      final sub = container.listen(sortedLeaguesProvider, (_, __) => count++);
      // Setup leagueDetail prerequisites
      final player1 = LeaguePlayer(id: 'p1', userId: 'u1', leagueId: 'l1', name: 'P1', avatarColorHex: 'FF0000');
      final player2 = LeaguePlayer(id: 'p2', userId: 'u2', leagueId: 'l1', name: 'P2', avatarColorHex: '00FF00');
      when(() => mockPlayerRepo.getByLeague('l1')).thenAnswer((_) async => [player1, player2]);
      when(() => mockPlayerRepo.get('p1')).thenAnswer((_) async => player1);
      when(() => mockPlayerRepo.get('p2')).thenAnswer((_) async => player2);
      when(() => mockMatchRepo.logSimpleMatch(match: any(named: 'match'))).thenAnswer((_) async => {});
      final newDate = DateTime(2023, 6, 15);
      final newMatch = SimpleMatch(id: 'm1', leagueId: 'l1', playedAt: newDate, isComplete: true, isDraw: false, sides: [Side(id: 's1', playerIds: ['p1']), Side(id: 's2', playerIds: ['p2'])], winnerSideId: 's1');
      when(() => mockMatchRepo.getAll()).thenAnswer((_) async => [newMatch]);
      when(() => mockMatchRepo.getByLeague('l1')).thenAnswer((_) async => [newMatch]);

      await container.read(leagueDetailProvider('l1').notifier).logSimpleMatch(winnerId: 'p1', loserId: 'p2', isDraw: false, playedAt: newDate);
      await Future.delayed(Duration.zero);
      expect(count, greaterThanOrEqualTo(1));
      final after = await container.read(sortedLeaguesProvider.future);
    ...
    test('updateSimpleMatch invalidates sortedLeaguesProvider', () async {
      ...
      var count = 0;
      final sub = container.listen(sortedLeaguesProvider, (_, __) => count++);
      final existing = SimpleMatch(id: 'm1', leagueId: 'l1', playedAt: DateTime(2023, 1, 1), isComplete: true, isDraw: false, sides: [Side(id: 's1', playerIds: ['p1']), Side(id: 's2', playerIds: ['p2'])], winnerSideId: 's1');
      final player1 = LeaguePlayer(id: 'p1', userId: 'u1', leagueId: 'l1', name: 'P1', avatarColorHex: 'FF0000');
      final player2 = LeaguePlayer(id: 'p2', userId: 'u2', leagueId: 'l1', name: 'P2', avatarColorHex: '00FF00');
      ...
      final updated = SimpleMatch(id: 'm1', leagueId: 'l1', playedAt: newDate, isComplete: true, isDraw: false, sides: [Side(id: 's3', playerIds: ['p1']), Side(id: 's4', playerIds: ['p2'])], winnerSideId: 's3');
      when(() => mockMatchRepo.getAll()).thenAnswer((_) async => [updated]);
      when(() => mockMatchRepo.getByLeague('l1')).thenAnswer((_) async => [updated]);

      await container.read(leagueDetailProvider('l1').notifier).updateSimpleMatch(matchId: 'm1', winnerId: 'p1', loserId: 'p2', isDraw: false, playedAt: newDate);
      await Future.delayed(Duration.zero);
      expect(count, greaterThanOrEqualTo(1));
      sub.close();
    });
    ...
    test('deleteMatch invalidates sortedLeaguesProvider', () async {
      final match = SimpleMatch(id: 'm1', leagueId: 'l1', playedAt: DateTime(2023, 6, 15), isComplete: true, isDraw: false, sides: [Side(id: 's1', playerIds: ['p1']), Side(id: 's2', playerIds: ['p2'])], winnerSideId: 's1');
      ...
      await container.read(leagueDetailProvider('l1').notifier).deleteMatch('m1');
      await Future.delayed(Duration.zero);
      expect(count, greaterThanOrEqualTo(1));
      final after = await container.read(sortedLeaguesProvider.future);
    ...
    test('preference toggle invalidates/re-sorts sortedLeaguesProvider (R3,R4)', () async {
      // Start with alphabetical false
      when(() => mockLeagueRepo.getAll()).thenAnswer((_) async => [tLeague1, tLeague2]); // Alpha, Beta
      when(() => mockMatchRepo.getAll()).thenAnswer((_) async => []);
      await container.read(sortedLeaguesProvider.future);
      var count = 0;
      final sub = container.listen(sortedLeaguesProvider, (_, __) => count++);
      // Switch to alphabetical descending via preference notifier
      await container.read(sortPreferenceProvider.notifier).setPreference(const LeagueSortPreference(mode: LeagueSortMode.alphabetical, descending: true));
    ...
    test('systematic failure visible: match repo error does not leak as silent empty', () async {
      // Already tested in unit, but integration expects sortedLeaguesProvider error state propagates
      when(() => mockMatchRepo.getAll()).thenThrow(Exception('DB failure systematic'));
